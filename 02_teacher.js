...
<div style="min-width:180px;">
  ...
  <input class="tIn" id="aPlaceIndex" type="number" min="1" placeholder="VD: 3">
        ...
        <div class="tRow" style="margin-top:10px;">
          ...
          <button class="btn ghost" id="aDownloadAssignTpl">Tải mẫu Excel</button>
...
const placeIndex = normalizePlaceIndex($("aPlaceIndex") ? $("aPlaceIndex").value : "") || "";
      ...
      // Download import templates (ZIP)
      const aDlTpl = $("aDownloadAssignTpl");
      if(aDlTpl){
        aDlTpl.onclick = ()=>{
          try{
            const url = new URL("mau_import_giao_bai_templates.zip", window.location.href).toString();
            const a = document.createElement("a");
            a.href = url;
            a.download = "MAU_IMPORT_GIAO_BAI.zip";
            document.body.appendChild(a);
            a.click();
            a.remove();
          }catch(e){
            console.error(e);
            toast("❌ Không tải được file mẫu. Vui lòng thử lại.");
          }
        };
      }
